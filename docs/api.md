...
This request will fail unless the account's email address and session has been verified.
...
Currently, each account is associated with exactly one email address. This address and the sessionToken must be "verified" before the account can be used (specifically, `/v1/certificate/sign` and `/v1/account/keys` will return errors until they are verified). In the future, this may be expanded to include multiple addresses, and/or alternate types of recovery methods (e.g., SMS). A new API will be provided for this extra functionality.
