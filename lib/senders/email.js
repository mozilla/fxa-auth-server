...
const clientName = require('./client_name')
...
module.exports = function (log) {
  ...
  Mailer.prototype.verifyLoginEmail = function (message) {
    ...
    return clientName.fetchClientData(message.service).then((clientData) => {
      const serviceHeader = translator.format(translator.gettext('New sign-in to %(serviceName)s'), {
        serviceName: clientData.name
      })
      const subject = translator.format(translator.gettext('Confirm new sign-in to %(serviceName)s'), {
        serviceName: clientData.name
      })
