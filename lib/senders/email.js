...
module.exports = function (log) {
  ...
  /**
   * Make given string safer for emails
   * @param str
   * @returns String
   */
  function makeSubjectSafer(str) {
    const SUBJECT_MAX_LENGTH = 100

    return str
      .trim()
      .substr(0, SUBJECT_MAX_LENGTH)
      .replace(/</g, '&lt;')
      .replace(/>/g, '&gt;');
  }
  ...
  Mailer.prototype.verifyLoginEmail = function (message) {
    ...
    return oauthClientInfo.fetch(message.service).then((clientInfo) => {
      ...
      const subject = makeSubjectSafer(translator.format(translator.gettext('Confirm new sign-in to %(clientName)s'), {
  ...
  Mailer.prototype.newDeviceLoginEmail = function (message) {
    ...
    return oauthClientInfo.fetch(message.service).then((clientInfo) => {
      const clientName = clientInfo.name
      const subject = makeSubjectSafer(translator.format(translator.gettext('New sign-in to %(clientName)s'), {
        clientName: clientName
      }))
