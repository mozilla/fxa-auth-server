...
module.exports = function (log, config) {
  ...
  Mailer.prototype.selectEmailServices = function (message) {
    ...
    return redis.get('config')
      ...
      .then(liveConfig => {
        return emailAddresses.reduce((promise, emailAddress) => {
          let isMatched, sendgrid, services, ses
          ...
          return promise
            .then(s => {
              ...
              if (liveConfig) {
                sendgrid = liveConfig.sendgrid
                ses = liveConfig.ses

                if (sendgrid) {
                  return isLiveConfigMatch(sendgrid, emailAddress)
                }
              }
            })
            .then(result => {
              ...
              if (isMatched) {
                return upsertServicesMapExternalSendgrid(services, emailAddress)
              }

              if (ses) {
                return isLiveConfigMatch(ses, emailAddress)
              }
            })
            .then(result => {
              if (isMatched) {
              ...
              if (result || config.emailService.forcedEmailAddresses.test(emailAddress)) {
                return upsertServicesMapExternalSes(services, emailAddress)
              }

              return upsertServicesMapInternal(services, emailAddress)
            })
        }, P.resolve(new Map()))
      })
    ...
    function upsertServicesMapExternalSendgrid (services, emailAddress) {
      return upsertServicesMap(services, SERVICES.externalSendgrid, emailAddress, {
    ...
    function upsertServicesMapExternalSes (services, emailAddress) {
      return upsertServicesMap(services, SERVICES.externalSes, emailAddress, {
        mailer: mailers.external,
        emailService: 'fxa-email-service',
        emailSender: 'ses'
      })
    ...
    function upsertServicesMapInternal (services, emailAddress) {
