...
module.exports = function (log, config) {
  ...
  // both sendgrid and ses, sendgrid wins. If a regex has a star height greater
  // than 1, the email address will be treated as a non-match without executing
  // the regex (to prevent us redosing ourselves).
  //
  ...
  Mailer.prototype.selectEmailServices = function (message) {
    const mailers = {
      internal: this.mailer,
      external: this.emailService
    }
    ...
    return redis.get('config')
      .catch(() => {})
      .then(liveConfig => {
        return Array.from(emailAddresses.reduce((services, emailAddress) => {
          if (liveConfig) {
            const { sendgrid, ses } = liveConfig

            if (sendgrid && isLiveConfigMatch(sendgrid, emailAddress)) {
              return upsertServicesArrayExternalSendgrid(services, emailAddress)
            }

            if (ses && isLiveConfigMatch(ses, emailAddress)) {
              return upsertServicesArrayExternalSes(services, emailAddress)
            }
          }
          ...
          return upsertServicesArrayInternal(services, emailAddress)
        }, new Map()).values())
      })
    ...
    function isLiveConfigMatch (liveConfig, emailAddress) {
      ...
      if (percentage >= 0 && percentage < 100) {
        ...
        for (const character in emailAddress) {
          sum += emailAddress.charCodeAt(character)
        }

        if (sum % 100 >= percentage) {
          return false
        }
      }

      if (regex && ! safeRegex(regex) || ! new RegExp(regex).test(emailAddress)) {
        return false
      }
    ...
    function upsertServicesArrayExternalSendgrid (services, emailAddress) {
      return upsertServicesArray(services, SERVICES.externalSendgrid, emailAddress, {
        mailer: mailers.external,
        emailService: 'fxa-email-service',
        emailSender: 'sendgrid'
      })
    }

    function upsertServicesArrayExternalSes (services, emailAddress) {
      return upsertServicesArray(services, SERVICES.externalSes, emailAddress, {
        mailer: mailers.external,
        emailService: 'fxa-email-service',
        emailSender: 'ses'
      })
    ...
    function upsertServicesArrayInternal (services, emailAddress) {
      return upsertServicesArray(services, SERVICES.internal, emailAddress, {
        mailer: mailers.internal,
        emailService: 'fxa-auth-server',
        emailSender: 'ses'
      })
    }

    function upsertServicesArray (services, service, emailAddress, data) {
