...
module.exports = function (log, config) {
  ...
  // both sendgrid and ses, sendgrid wins.
  //
  ...
  Mailer.prototype.selectEmailServices = function (message) {
    ...
    return redis.get('config')
      .catch(() => {})
      .then(liveConfig => {
        return emailAddresses.reduce((services, emailAddress) => {
          if (liveConfig) {
    ...
    function isLiveConfigMatch (liveConfig, emailAddress) {
      ...
      if (regex && ! new RegExp(regex).test(emailAddress)) {
        return false
      }
    ...
    function upsertServicesArray (services, service, emailAddress, data) {
      if (indices.has(service)) {
        services[indices.get(service)].emailAddresses.push(emailAddress)
      } else {
        indices.set(service, services.length)
        services.push({
