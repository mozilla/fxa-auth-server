...
module.exports = function (log, config) {
  ...
  //
  // Percentages stack sensibly. If, say, the following config is found:
  //
  // {
  //   sendgrid: {
  //     percentage: 10,
  ...
  // }
  //
  // ...a different 10% of mozilla email addresses will be mapped to each of
  // sendgrid and socketlabs, and 10% of all email addresses will be mapped
  // to ses.
  //
  ...
  Mailer.prototype.selectEmailServices = function (message) {
      ...
      .then(liveConfig => {
        return emailAddresses.reduce((promise, emailAddress) => {
          ...
          return promise
            .then(s => {
              services = s
              let globalConsumedPercentage = 0
              ...
              if (liveConfig) {
                return [ 'sendgrid', 'socketlabs', 'ses' ].reduce((promise, key) => {
                  ...
                  return promise
                    .then(() => {
                      if (senderConfig) {
                        return isLiveConfigMatch(senderConfig, emailAddress, globalConsumedPercentage, consumedPercentages)
                      }
                    })
                    .then(result => {
                      if (senderConfig && senderConfig.percentage) {
                        const { percentage, regex } = senderConfig
                        if (regex) {
                          if (consumedPercentages[regex]) {
                            consumedPercentages[regex] += percentage
                          } else {
                            consumedPercentages[regex] = percentage
                          }
                        } else {
                          globalConsumedPercentage += percentage
                        }
                      }
    ...
    function isLiveConfigMatch (liveConfig, emailAddress, globalConsumedPercentage, consumedPercentages) {
      return new P(resolve => {
        let { percentage } = liveConfig
        const { regex } = liveConfig

        if (regex) {
          if (consumedPercentages[regex]) {
            percentage += consumedPercentages[regex]
          }
        } else {
          percentage += globalConsumedPercentage
        }
