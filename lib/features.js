...
module.exports = config => {
  ...
  const pushNotificationForSignIn = config.pushNotificationForSignIn
  ...
  return {
    ...
    /**
     * Return whether or not push notification for sign in confirmation is
     * enabled for a given user, based on their uid and email address.
     *
     * @returns {boolean}
     */
    isPushLoginVerifyEnabled(uid, email) {
      if (pushNotificationForSignIn) {
        return pushNotificationForSignIn.enabled &&
          (isSampledUser(pushNotificationForSignIn.sampleRate, uid, 'pushNotificationForSignIn') ||
            pushNotificationForSignIn.enabledEmailAddresses.test(email))
      }
    },
