...
module.exports = config => {
  ...
  return {
    ...
    /**
     * Return whether an account meets the account created age requirement to
     * bypass sign-in confirmation.
     *
     * @returns {boolean}
     */
    isSigninBypassAccountWithAgeEnabled(account) {
      const bypassAccountWithAge = signinConfirmation.bypassAccountWithAge
      var accountCreatedSince = Date.now() - account.createdAt
      if (bypassAccountWithAge && bypassAccountWithAge.enabled
        && accountCreatedSince < bypassAccountWithAge.accountCreatedSinceMS) {
        return true
      }
