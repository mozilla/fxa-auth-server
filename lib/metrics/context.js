...
module.exports = function (log, config) {
  ...
  /**
   * Saves metrics context metadata against a token. Asynchronous,
   * returns a promise.
   *
   ...
   */
  function save (token, events, metadata) {
    if (! metadata) {
      return P.resolve()
    }
   ...
   */
  function copy (data, request, event) {
  ...
  function restore (token, event) {
    var memcached = getMemcached()
    return memcached.getAsync(getKey(token, event))
      .catch(function (err) {
  ...
  /**
   * Removes metrics context metadata for a token:event pair.
  ...
  function remove (token, event) {
    ...
    var memcached = getMemcached()
    return memcached.delAsync(getKey(token, event))
      .catch(function (err) {
...
function getKey (token, event) {
  return token.tokenId.toString('hex') + ':' + event
}
