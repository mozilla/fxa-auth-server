...
module.exports = function (log, config) {
  ...
  var compiledUARegexList
  if (config.metrics && config.metrics.allowedUARegex) {
    compiledUARegexList = config.metrics.allowedUARegex.map(function (re) {
      return new RegExp(re)
    })
  }

  var compiledEmailRegexList
  if (config.metrics && config.metrics.allowedEmailRegex) {
    compiledEmailRegexList = config.metrics.allowedEmailRegex.map(function (re) {
      return new RegExp(re)
    })
  }
  ...
  function validate(request) {
    ...
    if (compiledUARegexList && userAgent) {
      allowedUA = compiledUARegexList.some(function (re) {
    ...
    if (compiledEmailRegexList && email) {
      allowEmail = compiledEmailRegexList.some(function (re) {
