...
module.exports = (log, config) => {
  if (! log || ! Array.isArray(config.oauth.clientIds)) {
    throw new TypeError('Bad argument')
  }

  const SERVICES = new Map(config.oauth.clientIds)
  if (! SERVICES.has('sync')) {
    // Ensure there is an entry for Sync, which isn't identified by client id.
    SERVICES.set('sync', 'sync')
  }
  ...
  function getService (request) {
    const service = SERVICES.get(request.payload.service || request.query.service)
    if (service) {
      return {
        '$append': {
