...
var devicesCache = require('./routes/utils/devices_cache')
...
function create(log, error, config, routes, db, translator) {
  ...
  var useRedis = config.devicesCache.enabled
  var cache = {
    engine: useRedis ? require('catbox-redis') : require('catbox-memory')
  }
  if (useRedis) {
    cache.host = config.redis.host
    cache.port = config.redis.port
    cache.partition = config.devicesCache.keyPrefix
  }
  ...
  server.method('devices', devicesCache, {
    generateKey: function(req) {
      return req.auth.credentials.uid
    },
    cache: {
      expiresIn: config.devicesCache.expiresIn,
      generateTimeout: config.devicesCache.generateTimeout
    }
  })
