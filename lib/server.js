...
function create(log, error, config, routes, db) {
  ...
  function makeVerifiedCredentialFn (dbGetFn) {
    return function (id, cb) {
      log.trace({ op: 'DB.getToken', id: id })
      if (!HEX_STRING.test(id)) {
        return process.nextTick(cb.bind(null, null, null)) // not found
      ...
      dbGetFn(Buffer(id, 'hex'))
        .done(
          function (token) {
            if (token.tokenVerificationId) {
              return cb(error.unverifiedAccount())
            }
            return cb(null, token)
          },
          cb
  ...
  server.register(require('hapi-auth-hawk'), function (err) {
    ...
    server.auth.strategy(
      ...
      'hawk',
      {
        getCredentialsFunc: makeVerifiedCredentialFn(db.sessionTokenWithVerificationStatus.bind(db)),
        hawk: hawkOptions
      }
    )
    server.auth.strategy(
