...
async function create (log, error, config, routes, db, translator) {
  ...
  // Make it easier to debug internal errors by printing them.
  // Commented because it generates a lot of log chatter.
  // if (config.env !== 'prod') {
  //   serverOptions.debug = {request: ['error', 'uncaught']}
  // }
  ...
  server.ext('onPreAuth', (request, h) => {
    ...
    defineLazyGetter(request.app, 'uid', () => {
      // No magic here: we never get uid from request.payload.
      if (request.auth && request.auth.credentials) {
        return request.auth.credentials.uid || request.auth.credentials.user
      }
    })
  ...
  server.auth.strategy('refreshToken', 'fxa-oauth', {
    ...config.oauth,
    extra: {
      type: 'refresh_token',
    },
