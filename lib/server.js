    ...
    return {
      async authenticate (request, h) {
        try {
          ...
          const refreshToken = await oauthdb.checkRefreshToken(refreshTokenId, { client_id: request.payload.client_id })
          ...
          const devices = await db.accountDevices(refreshToken.sub)
          const device = devices.filter(device => device.id === refreshTokenId)[0]
          if (device) {
