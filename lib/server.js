...
function create(log, error, config, routes, db, translator) {
  ...
  const cookieSettings = config.verifiedUidCookie || {}
  server.state('verifiedUid', {
    ttl: cookieSettings.ttl || null,
    //clearInvalid: true,
    //ignoreErrors: true,
    //isSecure: false,
    isSameSite: false,
    path: '/',
    encoding: 'iron',
    password: cookieSettings.secret || 'THISISNOTASECRETTHISISNOTASECRET'
  })
