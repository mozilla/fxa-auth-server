...
var create =  async (log, error, config, routes, db, translator) => {
  ...
  promises.push(server.register(require('inert')))
...
//register auth-hawk
promises.push(
  server.register(require('hapi-auth-hawk'))
  .then(() => {
    ...
    server.register(require('hapi-fxa-oauth'))
    .then(() => {
