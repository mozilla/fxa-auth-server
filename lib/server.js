...
async function create (log, error, config, routes, db, oauthdb, translator) {
  ...
  server.auth.scheme('fxa-oauth-refreshToken', (server, options) => {
    return {
      async authenticate (request, h) {
        try {
          const refreshTokenId = request.headers.authorization.split(' ')[1];
          //const refreshTokenId = request.payload.refresh_token;
          const refreshToken = await oauthdb.checkRefreshToken(refreshTokenId)
          if (! refreshToken || ! refreshToken.active) {
            return h.unauthenticated()
          }

          // Create a mock sessionToken that can be used in functions expect one.
          const sessionToken = {
            uid: refreshToken.sub,
            id: refreshTokenId,
            tokenVerified: true // TODO - not really sure about this.
          }

          console.log('refreshToken', refreshToken)

          const devices = await db.devices(refreshToken.sub)
          console.log('devices', devices, refreshTokenId)
          const device = devices.filter(device => device.sessionToken === refreshTokenId)[0]
          if (device) {
            console.log('bobotest', device.id, refreshTokenId)
            sessionToken.deviceId = device.id
            sessionToken.deviceName = device.name
            sessionToken.deviceType = device.type
            sessionToken.deviceCreatedAt = device.createdAt
            sessionToken.deviceCallbackURL = device.callbackURL
            sessionToken.deviceCallbackPublicKey = device.callbackPublicKey
            sessionToken.deviceCallbackAuthKey = device.callbackAuthKey
            sessionToken.deviceCallbackIsExpired = device.callbackIsExpired
            sessionToken.deviceAvailableCommands = device.availableCommands
          }
