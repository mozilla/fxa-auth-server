...
module.exports = (
  ...
  const redis = require('./redis-pool')(config, log)
  const {
    SessionToken,
    KeyFetchToken,
    AccountResetToken,
  ...
  function DB(options) {
    ...
    if (redis) {
      redis.acquire()
        // This is the main redis connection. All gets, sets and dels can be called on it.
        // But updates have side-effects so should be performed on single-use connections,
        // acquired especially for that purpose. Attempting to update concurrently on a
        // single connection will fail, even if the updates are for different keys.
        .then(result => this.redis = result)
    }
  }
  ...
  DB.prototype.updateSessionToken = function (token, geo) {
    ...
    return geo
      ...
      .catch(() => {})
      .then(() => safeRedisUpdate(uid, sessionTokens => {
        if (sessionTokens) {
  ...
  DB.prototype.deleteSessionToken = function (sessionToken) {
    ...
    return P.resolve()
      .then(() => {
        if (redis) {
          return safeRedisUpdate(uid, sessionTokens => {
            if (! sessionTokens) {
              return
            }
  ...
  function safeRedisGet (connection, key) {
  ...
  function safeRedisUpdate (key, getUpdatedValue) {
    return P.using(
      redis.acquire().disposer(connection => redis.release(connection)),
      connection => connection.update(key, getUpdatedValue)
    )
  }
