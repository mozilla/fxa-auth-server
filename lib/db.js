...
module.exports = (
  ...
  function toClientInstance(dbRow) {
    delete dbRow.uid
    return dbRow
  }

  SAFE_URLS.deleteClientInstance = new SafeUrl('/account/:uid/client_instance/:id', 'db.deleteClientInstance')
  DB.prototype.deleteClientInstance = async function (uid, id) {
    log.trace({ op: 'DB.deleteClientInstance', uid, id })
    return this.pool.del(SAFE_URLS.deleteClientInstance, { uid, id })
  }

  SAFE_URLS.clientInstance = new SafeUrl('/account/:uid/client_instance/:id', 'db.clientInstance')
  DB.prototype.clientInstance = async function (uid, id) {
    log.trace({ op: 'DB.clientInstance', uid, id })
    try {
      const row = await this.pool.get(SAFE_URLS.clientInstance, { uid, id })
  ...
  SAFE_URLS.clientsInstances = new SafeUrl('/account/:uid/clients_instances', 'db.clientsInstances')
  DB.prototype.clientsInstances = async function (uid) {
    log.trace({ op: 'DB.clientsInstances', uid })
    const rows = await this.pool.get(SAFE_URLS.clientsInstances, { uid })
    return rows.map(toClientInstance)
  }

  SAFE_URLS.upsertClientInstance = new SafeUrl('/account/:uid/client_instance/:instanceId', 'db.upsertClientInstance')
  DB.prototype.upsertClientInstance = async function (uid, instanceId, clientId, metadata) {
    log.trace({ op: 'DB.upsertClientInstance', uid, instanceId, clientId })
    await this.pool.post(
      SAFE_URLS.upsertClientInstance,
      { uid, instanceId },
      {
        clientId,
        name: metadata.name,
        ...
        pushAuthKey: metadata.pushAuthKey,
