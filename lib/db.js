...
module.exports = (
  ...
  DB.prototype.consumeUnblockCodeForEmail = function (email, code) {
    log.trace({
      op: 'DB.consumeUnblockCodeForEmail',
      email: email
    })
    // XXX TODO: hack for now, add proper implementation later.
    return this.accountRecord(email).then((accountRecord) => {
      const uid = accountRecord.uid
      return this.pool.del('/account/' + uid + '/unblock/' + code)
        .catch(
          function (err) {
            if (isNotFoundError(err)) {
