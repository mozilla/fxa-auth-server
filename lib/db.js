...
module.exports = (
  ...
  const MAX_ATTEMPTS_FOR_UNBLOCK_CODE = config.signinUnblock.maxAttempts
  ...
  DB.prototype.createUnblockCode = function (uid) {
    ...
    return UnblockCode()
      .then(
        (unblock) => {
          return this.pool.put('/account/' + uid + '/unblock/' + unblock + '/maxAttempts/' + MAX_ATTEMPTS_FOR_UNBLOCK_CODE)
            .then(
              () => {
                return unblock
              },
  ...
  DB.prototype.tryUnblockCode = function (uid, code) {
    log.trace({
      op: 'DB.tryUnblockCode',
      uid: uid
    })
    return this.pool.put('/account/' + uid + '/unblock/' + code + '/try')
      .catch(
        function (err) {
          if (isNotFoundError(err)) {
            throw error.invalidUnblockCode()
