...
module.exports = function (
  ...
  DB.prototype.ssoRecord = function (ssoId) {
    log.trace({ op: 'DB.ssoRecord', ssoId: ssoId })
    return this.pool.get('/ssoRecord/' + Buffer(ssoId, 'utf8').toString('hex'))
      .then(
        function (body) {
          var data = bufferize(body)
          data.emailVerified = !!data.emailVerified
        ...
        function (err) {
          if (err.statusCode === 404) {
            err = error.unknownAccount(ssoId)
          }
          throw err
        }
      )
