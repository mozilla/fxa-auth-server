...
module.exports = function (log, error) {
  ...
  return function start(bounceQueue, db) {
    ...
    function logBounceEvent(message) {
      var verifyCode = getHeaderValue('X-Verify-Code', message)
      var uid = getHeaderValue('X-Uid', message)
      ...
      if (uid && verifyCode) {
        ...
        return log.activityEvent('account.confirmation.bounced', fakeRequestObject, {
          uid: uid
        })
      }
    }
    ...
    function handleBounce(message) {
      ...
      return P.each(recipients, function (recipient) {
        ...
        return findEmailRecord(email)
          ...
          .then(logBounceEvent.bind(null, message))
      }).then(
        function () {
