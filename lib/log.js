...
var amplitude = new Amplitude(config.get('amplitude.apiKey'))

var amplitudeLastSeenTimestamp = 0
var amplitudeEventCounter = 0
var amplitudeHmacKey = config.get('amplitude.hmacKey')
...
function sendToAmplitude(data) {
  ...
  userAgent.call(data, data.userAgent, this)
  amplitude.track({
    event_type: data.event,
    event_id: nextAmplitudeEventId(data),
    device_id: data.flow_id ? anonymizeForAmplitude(data.flow_id) : undefined,
    user_id: data.uid ? anonymizeForAmplitude(data.uid) : undefined,
    time: data.time,
    // Use flowBeginTime as numeric session identifier
    session_id: data.flow_time ? data.time - data.flow_time : undefined,
    event_properties: {
      flow_time: data.flow_time,
      service: data.service
    },
    os_name: data.uaOS,
    os_version: data.uaOSVersion,
    platform: data.uaBrowser
  })
}
...
Lug.prototype.activityEvent = function (data) {
  ...
  // XXX TODO: need to avoid sending the event twice
  // if it's both an activityEvent and a flowEvent...
