...
function Lug(options) {
  ...
  const createLogger = mozlog({
    app: this.name,
    level: options.level,
    stream: options.stderr ? new Stream(options.stderr, new Error('trace')) : process.stderr, // options.stderr || process.stderr,
    fmt: options.fmt,
    uncaught: options.uncaught || 'exit'
  })
...
class Stream {
  constructor(target, trace) {
    this.target = target
    this.trace = trace;
  }
  write(data, enc, cb) {
    console.log('lug', data)
    console.log('lug trace', this.trace.stack)
    this.target.write(data, enc, cb)
  }
}
