...
var amplitude = new Amplitude('b63fd23a7571e9aaa9970788d376f389')
...
Lug.prototype.flowEvent = function (data) {
  ...
  try {
    var ampEv = {
      event_type: data.event, // required
      device_id: data.flow_id, // only required if device id is not passed in
      event_properties: {
        time: data.time,
        flow_time: data.flow_time
      }
    }
    amplitude.track(ampEv)
  } catch (e) {
    console.log('wow', e);
  }
