...
var PUSH_COMMANDS = {
  ...
  VERIFY_LOGIN_NOTIFICATION: 'fxaccounts:verify_login'
}
...
var TTL_VERIFY_PUSH = TTL_PASSWORD_CHANGED
...
var reasonToEvents = {
  ...
  verifyLogin: {
    send: 'push.verify_login.send',
    success: 'push.verify_login.success',
    resetSettings: 'push.verify_login.reset_settings',
    failed: 'push.verify_login.failed',
    noCallback: 'push.verify_login.no_push_callback',
    noKeys: 'push.verify_login.data_but_no_keys'
  },
  devicesNotify: {
...
module.exports = function (log, db, config) {
  ...
  return {
    ...
    /**
     * Notifies connected devices about a signin confirmation request.
     *
    */

    notifyVerifyLogin: function notifyVerifyLogin(uid, devices, geoData, tokenVerificationId) {
      var data = Buffer.from(JSON.stringify({
        command: PUSH_COMMANDS.VERIFY_LOGIN_NOTIFICATION,
        data: {
          title: 'Sign-in Request',
          body: 'Please confirm your Sign-in',
          url: config.contentServer.url + '/complete_2fa?code=' + tokenVerificationId + '&uid=' + uid
        }
      }))
      var options = { data: data, TTL: TTL_VERIFY_PUSH }
      return this.sendPush(uid, devices, 'verifyLogin', options)
    },
