...
const TTL_MESSAGE_RECEIVED = 1 * 3600 // 1 hour
...
module.exports = function (log, db, config) {
  ...
  return {
    ...
    /**
     * Notify devices that a new message is ready to be retrieved.
     *
     * @param {String} uid
     * @param {Device} device
     * @param {Device} url - url to retrieve the message
     * @param {String} topic
     * @param {String} reason
     * @promise
     */
    notifyMessageReceived(uid, device, url, topic, reason) {
      const options = {
        data: {
          url,
          topic
        },
        TTL: TTL_MESSAGE_RECEIVED
      }
      return this.sendPush(uid, [device], reason, options)
    },
