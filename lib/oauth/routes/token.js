...
const config = require('../../../config').get('oauth');
const db = require('../db');
...
const MAX_TTL_S = config.expiration.accessToken / 1000;
const GRANT_AUTHORIZATION_CODE = 'authorization_code';
const GRANT_REFRESH_TOKEN = 'refresh_token';
const GRANT_JWT = 'urn:ietf:params:oauth:grant-type:jwt-bearer';

const JWT_AUD = config.publicUrl + '/v1/token';

const SERVICE_CLIENTS = {};
const SERVICE_JWTOOL = new JwTool(config.serviceClients.map(function(client) {
...
const ID_TOKEN_EXPIRATION = Math.floor(config.openid.ttl / 1000);
const ID_TOKEN_ISSUER = config.openid.issuer;
const ID_TOKEN_KEY = JwTool.JWK.fromObject(config.openid.key, {
  iss: ID_TOKEN_ISSUER
});

const REFRESH_LAST_USED_AT_UPDATE_AFTER_MS = config.refreshToken.updateAfter;
...
function confirmCode(id, code) {
  return db.getCode(buf(code)).then(function(codeObj) {
    if (! codeObj) {
      ...
      var expiresAt = +codeObj.createdAt + config.expiration.code;
