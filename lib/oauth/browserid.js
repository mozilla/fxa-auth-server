...
const AUDIENCE = config.get('oauth.publicUrl');

const request = require('request').defaults({
  url: config.get('oauth.browserid.verificationUrl'),
  pool: {
    maxSockets: config.get('oauth.browserid.maxSockets')
  }
});
...
module.exports = function verifyAssertion(assertion) {
  ...
  request.post(opts, (err, res, body) => {
    ...
    const allowedIssuer = config.get('oauth.browserid.issuer');
