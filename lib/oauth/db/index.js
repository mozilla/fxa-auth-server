...
const klass = config.get('oauth.db.driver') === 'mysql' ?
...
function preClients() {
  var clients = config.get('oauth.clients');
  if (clients && clients.length) {
    ...
    return P.all(clients.map(function(c) {
      ...
      if (! config.get('oauth.db.autoUpdateClients')) {
        return P.resolve();
      }
...
function serviceClients() {
  var clients = config.get('oauth.serviceClients');
  if (clients && clients.length) {
 ...
 */
function scopes() {
  var scopes = config.get('oauth.scopes');
...
function withDriver() {
  ...
  var p;
  if (config.get('oauth.db.driver') === 'mysql') {
    p = klass.connect(config.get('oauth.mysql'));
  } else {
    p = klass.connect();
  }
  return p.then(function(store) {
    logger.debug('connected', { driver: config.get('oauth.db.driver') });
    driver = store;
