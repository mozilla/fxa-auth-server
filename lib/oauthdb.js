...
function mapOAuthError(log, err) {
  ...
  switch (err.errno) {
    ...
    default:
      ...
      if (err.validation && err.validation.keys) {
        log.warn({
          op: 'oauthdb.mapOAuthError.validation',
          err: err,
          name: 'validation error',
          errno: err.errno,
          warning: err.validation.keys
        })
      }
...
module.exports = (log, config) => {
  ...
  return {
    ...
    async postTokenCodeExchange(oauthParams) {
      console.log('huhhh', oauthParams)
      try {
        return await api.postTokenCodeExchange(oauthParams)
      } catch (err) {
        throw mapOAuthError(log, err)
      }
    },
