...
module.exports = (log, db, mailer, config, customs, push) => {
  ...
  const USER_IP_VERIFICATION_KEY = config.userIpJwtVerification.key
  return [
    {
      method: 'GET',
    ...
    },
    {
      ...
      config: {
        validate: {
          ...
          payload: {
            ...
            userIpToken: isA.string().optional(),
            service: validators.service,
            reminder: isA.string().max(32).alphanum().optional(),
            type: isA.string().max(32).alphanum().optional(),
      ...
      handler (request, reply) {
        ...
        const userIpToken = request.payload.userIpToken
        const service = request.payload.service || request.query.service
        ...
        return db.account(uid)
          .then((account) => {
            return verifyIpToken(userIpToken)
              .catch((err) => {
                throw error.jwtVerificationError(err)
              })
              // This endpoint is not authenticated, so we need to look up
              // the target email address before we can check it with customs.
              .then((ip) => customs.check(request, account.email, 'recoveryEmailVerifyCode', ip))
              .then(() => account)
          })
          .then((account) => {
        ...
        function verifyIpToken(token) {
          return new Promise((resolve, reject) => {
            if (token) {
              jwt.verify(token, USER_IP_VERIFICATION_KEY, (err, decodedInfo) => {
                if (err) {
                  reject(err)
                } else {
                  resolve(decodedInfo.ip)
                }
              })
            } else {
              resolve()
