    ...
    },
    {
      ...
      handler (request, reply) {
        ...
        customs.check(request, primaryEmail, 'deleteEmail')
          ...
          .then(deleteEmail)
          .then(() => {
            const resolvedEmail = account.emails.filter((item) => {
              if (item.normalizedEmail === email.toLowerCase() && item.isVerified) {
                return item
              }
            })

            // Don't notify the secondary email that it has been removed from the account.
            // We only want primary email and all *other* verified emails to get this.
