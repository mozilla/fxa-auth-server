  ...
  var routes = [
    {
      ...
      handler: function certificateSign(request, reply) {
        ...
        return P.resolve()
          ...
          .then(
            function () {
              // Set a cookie to remember the user if we saw a verified session.
              // This might help them avoid a confirmation loop when signing
              // in again on the same device.
              reply(certResult)
                .state('verifiedUid', sessionToken.tokenVerified ? sessionToken.uid : null)
            },
            reply
          )
      }
