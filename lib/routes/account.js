  ...
  var routes = [
    ...
    },
    {
      ...
      config: {
        validate: {
          payload: {
            ...
            unblockCode: isA.string().optional(),
            metricsContext: metricsContext.schema
          }
        },
        response: {
      ...
      handler: function (request, reply) {
        ...
        function checkUnblockCode (e) {
          // if  e == THROTTLED || e == BLOCKED && e.canUnblock
          //   if unblockCode
          //     return db.unblock(unblockCode)
          //       then return customs.unblocked(request)
          //var unblockCode = request.payload.unblockCode
          // cannot unblock this error
          throw e
        }
    ...
    },
    {
      ...
      handler: function (request, reply) {
        ...
        return P.resolve()/*customs.check(request, email, 'sendUnblockCode')*/
          .then(lookupAccount)
    ...
    },
    {
      ...
      config: {
        validate: {
          payload: {
            uid: isA.string().max(32).regex(HEX_STRING).required(),
            unblockCode: isA.string().max(8).required()
          }
        }
      },
      handler: function (request, reply) {
        var uid = Buffer(request.payload.uid, 'hex')
        var unblockCode = Buffer(request.payload.unblockCode, 'hex')

        log.begin('Account.RejectUnblockCode', request)
        db.consumeUnblockCode(uid, unblockCode)
          .then(
            function () {
              return {}
            }
