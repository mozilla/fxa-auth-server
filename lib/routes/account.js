          ...
          .then((emailRecord) => {
            return totpUtils.hasTotpToken(emailRecord)
              .then((hasToken) => {
                ...
                // Ensure that this session has been verified with the correct assurance level for account
                // if it has TOTP enabled
                if (sessionToken && hasToken && (sessionToken.tokenVerificationId || sessionToken.authenticatorAssuranceLevel <= 1)) {
                  throw error.unverifiedSession()
                }

                // If TOTP is not enabled, we fall back to the default behavior and let the user
                // delete the account
                return emailRecord
              })
          })
