          ...
          return db.emailRecord(email)
            .then(
              function (result) {
                ...
                doSigninConfirmation = requestHelper.shouldEnableSigninConfirmation(emailRecord.uid.toString('hex'), config)
    ...
    },
    {
      ...
      handler: function (request, reply) {
        ...
        function createResponse() {
          ...
          var doSigninConfirmation = requestHelper.shouldEnableSigninConfirmation(sessionToken.uid.toString('hex'), config)

          if (doSigninConfirmation) {
            return db.sessionTokenWithVerificationStatus(sessionToken.id)
              .then(function (token) {

                var sessionVerified = token.tokenVerified
            ...
            // If sign-in confirmation is disabled, fall back to account
            // considered verified if email verified
            return {
      ...
      handler: function (request, reply) {
        ...
        return db.sessionTokenWithVerificationStatus(sessionToken.id)
          .then(function (token) {
            ...
            var doSigninConfirmation = requestHelper.shouldEnableSigninConfirmation(sessionToken.uid.toString('hex'), config)
            var func
            if (!token.emailVerified || !doSigninConfirmation) {
            ...
            return customs.check(
              ...
              .then(func.bind(
                mailer,
                sessionToken,
                doSigninConfirmation ? token.tokenVerificationId : sessionToken.emailCode, // Send tokenVerficationId, unless sign-in confirmation is disabled
                userAgent.call({
                  service: service,
                  timestamp: Date.now(),
          ...
          .then(
            function (account) {
              ...
              return db.verifyToken(code, account)
                .then(function () {
                  log.event('tokenVerified', request, {
                    email: account.email,
                    uid: account.uid,
                    locale: account.locale,
