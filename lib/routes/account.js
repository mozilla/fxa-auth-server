...
module.exports = (log, db, mailer, Password, config, customs, signinUtils, push, clients) => {
      ...
      handler: async function accountReset(request) {
        ...
        function resetAccountData () {
          ...
          return random.hex(32)
            ...
            .then(() => {
              // Notify the targets that the account has changed.
              clients.accountPushClientsAndDevices(accountResetToken.uid).then(targets => push.notifyPasswordReset(accountResetToken.uid, targets))
          ...
          .then((emailRecord) => {
            ...
            return signinUtils.checkPassword(emailRecord, password, request.app.clientAddress)
              .then((match) => {
                ...
                // We fetch the targets to notify before deleteAccount()
                // because obviously we can't retrieve the targets list after!
                return clients.accountPushClientsAndDevices(uid)
              })
              .then((targets) => {
                targetsToNotify = targets
                return db.deleteAccount(emailRecord)
                  .then(() => log.info({ op: 'accountDeleted.byRequest', ...emailRecord }))
              })
              .then(() => {
                push.notifyAccountDestroyed(uid, targetsToNotify)
                  .catch(() => {
