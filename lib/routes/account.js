  ...
  var routes = [
    ...
    },
    {
      ...
      handler: function (request, reply) {
        ...
        let isAccountVerification = false
        let account
        let verifiedEmail
        let device
        ...
        function checkIsAccountVerifyCode() {
          isAccountVerification = butil.buffersAreEqual(code, account.emailCode)
          return
        }

        function checkIsSecondaryEmailVerifyCode() {
          if (! isAccountVerification) {
            // Check to see if this code belongs to an email in the emails table
            const secondaryEmails = features.isSecondaryEmailEnabled() ? db.accountEmails(account.uid) : P.resolve([])
            return P.all([secondaryEmails])
              .spread((emails) => {
        ...
        function verifySecondaryEmailCode() {
        ...
        function verifyToken() {
          // Skip verifying token if this was an email verification code
          if (isEmailVerification) {
            return
          }
        ...
        function verifyAccount() {
          // Skip verifying account if this was an email verification code
          if(isEmailVerification) {
            return
          }
        ...
        return getAccount()
          .then(checkIsAccountVerifyCode)
          .then(checkIsSecondaryEmailVerifyCode)
          .then(verifySecondaryEmailCode)
          .then(getDevice)
          .then(verifyToken)
          .then(verifyAccount)
          .then(
