    ...
    },
    {
      ...
      handler: function (request, reply) {
        ...
        function createSessionToken () {
          ...
          // Flag that session needs to be validated if requesting keys
          if (request.query.keys === 'true') {
            sessionTokenOptions.tokenVerified = false;
            sessionTokenOptions.tokenVerificationId = tokenVerificationId;
          }
          ...
          return password.unwrap(emailRecord.wrapWrapKb)
            ...
            .then(
              function (keyFetchToken) {
                ...
                // Send sign-in confirmation email and set challenge to `signin.email`
                // to signify that this sign-in attempt needs to be verified via email.
                if (!sessionToken.tokenVerified && response.verified === true) {
                  response.challenge = 'signin.email';

                  var service = request.payload.service || request.query.service
                  mailer.sendConfirmLoginEmail(sessionToken, tokenVerificationId, {
                    service: service
                  });
                }
