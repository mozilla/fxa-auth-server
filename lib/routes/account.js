...
module.exports = function (
  ...
  var routes = [
    ...
    {
      method: 'POST',
      path: '/account/login/send_authorization_code',
      config: {
        validate: {
          payload: {
            email: validators.email().required(),
      ...
      handler: function (request, reply) {
        log.begin('Account.sendAuthorizationCode', request)

        var authorizationCode = crypto.randomBytes(16)
        var form = request.payload
        var email = form.email
        var service = request.payload.service || request.query.service
        var redirectTo = request.payload.redirectTo
        ...
        return db.emailRecord(email)
          .then(
            function (account) {
              return mailer.sendAuthorizeLoginEmail(
                account,
                authorizationCode,
                {
                  acceptLanguage: request.app.acceptLanguage,
    ...
    {
      method: 'POST',
      path: '/account/login/reject_authorization_code',
      config: {
        validate: {
          payload: {
    ...
    {
      method: 'POST',
      path: '/account/login/verify_authorization_code',
      config: {
        validate: {
          payload: {
            uid: isA.string().max(32).regex(HEX_STRING).required(),
