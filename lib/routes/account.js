  ...
  var routes = [
    ...
    },
    {
      ...
      config: {
        ...
        response: {
          // basically we have a tagged union response
          //
          // for legacy reasons, we need to always include the verified booleans
          // some clients specifically check for `verified !== false`

          // an array here means Joi.alternatives()
          schema: [
            {
              status: 'pending',
      ...
      handler: function (request, reply) {
        ...
        function checkStatus() {
          ...
          if (! isVerified) {
            return bounces.check(sessionToken.email)
              .catch(e => {
                switch (e.errno) {
