    ...
    },
    {
      ...
      handler: function (request, reply) {
        ...
        function sendVerifyLoginEmail() {
          if (doSigninConfirmation) {
            log.info({
              op: 'account.signin.confirm.email.start',
              uid: accountRecord.uid,
              tokenVerificationId: tokenVerificationId
            })
        ...
        function sendVerifyLoginPush(devices) {
          if (doSigninConfirmation) {
            log.info({
              op: 'account.signin.confirm.push.start',
              uid: accountRecord.uid,
              tokenVerificationId: tokenVerificationId
            })
            ...
            return getGeoData(ip)
              .then(
                function (geoData) {
                  return push.notifyVerifyLogin(accountRecord.uid, devices, geoData, tokenVerificationId)
                })
          }
        }

        function sendVerifyLogin() {
          if (features.isPushLoginVerifyEnabled(accountRecord.uid, accountRecord.email)) {
            return db.devices(accountRecord.uid).then(
              devices => {
                devices = devices.filter(function(device) {
                  return (device.type === 'desktop' && parseFloat(device.uaBrowserVersion) >= 56)
                })
                if (devices.length === 0) {
                  return sendVerifyLoginEmail()
                } else {
                  return sendVerifyLoginPush(devices)
                }
              }
            )
          }
              ...
              return db.devices(accountRecord.uid).then(
                devices => {
                  ...
                  if (devices.length === 0) {
                    ...
                    response.verified = false
                    response.verificationMethod = 'push'
                    response.verificationReason = 'login'
                  }
                  return P.resolve(response)
                }
