  ...
  var routes = [
      ...
      handler: function (request, reply) {
        ...
        return customs.check(request, sessionToken.email, 'recoveryEmailResendCode')
          ...
          .then(() => {
            ...
            if (email) {
              // Ensure that the verification email is sent to the correct email
              mailerOpts.email = email
            }

            return func(sessionToken, code, mailerOpts)
          })
        ...
        function setVerifyCode() {
          if (email) {
            ...
            // an additional email. This should return the corresponding email code for verification.
            return db.accountEmails(sessionToken.uid)
              .then((emails) => {
                return emails.some((userEmail) => {
        ...
        db.account(uid)
          ...
          .then(() => {
            ...
            if (! isAccountVerification) {
              // Check to see if this code belongs to an email, at this point the code
              // can only be a sign-in confirmation code or email code.
              return db.accountEmails(account.uid)
                .then((emails) => {
                  isEmailVerification = emails.some((email) => {
                    if (code.toString('hex') === email.emailCode) {
                      return true
                    }
                  })
                })
                .then(() => {
                  ...
                  // To help reduce complexity, if the code belongs to an email,
                  // verify it.
                  if (isEmailVerification) {
                    return db.verifyEmail(account, code)
                      .then(function () {
                        log.info({
                          op: 'account.verifyEmail.additional.confirmed',
                          uid: uidHex,
    ...
    },
    {
      ...
      config: {
        ...
        response: {
          schema: isA.array().items(
            isA.object({
              isVerified: isA.boolean().required(),
      ...
      handler: function (request, reply) {
        ...
        log.begin('Account.RecoveryEmailEmail', request)

        customs.check(request, userEmail, 'accountEmails')
          .then(getAccountEmails)
          .then(createResponse)
          .done(reply, reply)
    ...
    },
    {
      ...
      handler: function (request, reply) {
        ...
        customs.check(request, userEmail, 'createEmail')
          .then(generateRandomValues)
          .then(createEmail)
        ...
        function createEmail() {
          ...
          return db.createEmail(uidHex, emailData)
        }
        ...
        function sendEmailVerification() {
          return getGeoData(ip)
            .then((geoData) => {
              ...
              return mailer.sendVerifySecondaryEmail(account, emailCode, userAgent.call({
                acceptLanguage: request.app.acceptLanguage,
                email: email,
                userEmail: userEmail,
                ip: ip,
    ...
    },
    {
      ...
      handler: function (request, reply) {
        ...
        customs.check(request, userEmail, 'deleteEmail')
          .then(deleteEmail)
          .done(reply, reply)

        function deleteEmail() {
          return db.deleteEmail(uidHex, email.toLowerCase())
        }
      }
    },
    {
