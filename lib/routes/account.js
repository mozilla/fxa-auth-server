  ...
  var routes = [
    {
      ...
      handler: function accountCreate(request, reply) {
        ...
        function preVerifyAccount () {
          if (preVerified) {
            return db.verifyTokens(tokenVerificationId, account)
          }
        }
    ...
    },
    {
      ...
      handler: function (request, reply) {
        ...
        var emailRecord, sessionToken, device, keyFetchToken
        var doSigninConfirmation = shouldEnableSigninConfirmation()
        ...
        function shouldEnableSigninConfirmation() {
          // Signin confirmation is determined by whether the feature is enabled and
          // if it falls within config sample rate
          if (config.signinConfirmation.enabled) {
            var rand = Math.floor(Math.random() * (100 + 1))
            return rand < (config.signinConfirmation.sample_rate * 100)
          } else {
            return false
          }
        }
        ...
        function sendVerifyLoginEmail() {
          if (wantsKeys(request) && doSigninConfirmation) {
            // The response doesn't have to wait for this,
            // so we don't return the promise.
        ...
        function createResponse() {
          ...
          if (doSigninConfirmation) {
            ...
            return db.verifyTokens(tokenVerificationId, sessionToken)
              .then(function () {
                return response
              })
          }
      ...
      handler: function (request, reply) {
        ...
        openid.verifyAssertion(
          ...
          function (err, assertion) {
            ...
            var locale = request.app.acceptLanguage
            var tokenVerificationId = crypto.randomBytes(32)
