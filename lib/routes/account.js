    ...
    },
    {
      ...
      config: {
        ...
        response: {
          schema: isA.array().items(isA.object({
            name: isA.string().max(255).required().allow(''),
            createdAt: isA.number(),
            createdAtTimeFormatted: isA.string(),
            verified: isA.boolean()
          }))
        }
      },
      handler: function (request, reply) {
        log.begin('Account.security.events', request)
        var sessionToken = request.auth.credentials
        var uid = sessionToken.uid
        ...
        return db.securityEvents({
          ...
          events.map(function(event) {
            event.createdAtTimeFormatted = localizeTimestamp.format(event.createdAt,
              request.headers['accept-language'])
          })
          reply(events)
        }, reply)
