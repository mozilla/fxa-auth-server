  ...
  var routes = [
    {
      ...
      handler: function accountCreate(request, reply) {
        ...
        function preVerifyAccount () {
          if (preVerified) {
            return db.verifyToken(tokenVerificationId, account)
          }
        }
    ...
    },
    {
      ...
      handler: function (request, reply) {
        ...
        function shouldEnableSigninConfirmation(uid) {
          ...
          if (config.signinConfirmation.enabled) {

            // Returns the first 2 integers of uid
            var uidNum = uid.match(/\d/g).splice(0, 2).join('')
          ...
          return db.emailRecord(email)
            .then(
              function (result) {
                ...
                doSigninConfirmation = shouldEnableSigninConfirmation(emailRecord.uid.toString('hex'))
        ...
        function sendVerifyLoginEmail() {
          ...
          // created verified.
          var shouldSendVerifyLoginEmail = wantsKeys(request) && doSigninConfirmation
    ...
    },
    {
      ...
      handler: function (request, reply) {
        ...
        function createResponse() {
          return db.sessionTokenWithVerificationStatus(sessionToken.id)
            .then( function(token) {

              var sessionVerified = !token.tokenVerificationId // tokenVerificationId exists for unverified sessions
              var emailVerified = !!token.emailVerified
      ...
      handler: function (request, reply) {
        ...
        if (!sessionToken.emailVerified) {
          ...
          return customs.check(
            ...
            .then(mailer.sendVerifyCode.bind(
              mailer,
              sessionToken,
          ...
          return db.sessionTokenWithVerificationStatus(sessionToken.id)
            .then(function(token) {
              ...
              return customs.check(
                ...
                sessionToken.email,
                'recoveryEmailResendCode')
                .then(mailer.sendVerifyLoginEmail.bind(
                    mailer,
                    sessionToken,
        ...
        db.account(Buffer(uid, 'hex'))
          .then(
            function (account) {
               ...
               */
              return db.verifyToken(code, account)
                .then(function () {
                  log.event('tokenVerified', {
                    email: account.email,
                    uid: account.uid,
                    locale: account.locale,
                    tokenVerificationId: code
                  })
                })
                .catch(function (err) {
                  if (err.errno === error.ERRNO.INVALID_TOKEN && butil.buffersAreEqual(code, account.emailCode)) {
    ...
    },
    {
      ...
      config: {
        ...
        validate: {
          payload: {
            authPW: isA.string().min(64).max(64).regex(HEX_STRING).required(),
            metricsContext: metricsContext.schema,
            sessionToken: isA.string().min(64).max(64).regex(HEX_STRING).required()
          }
        }
      },
      handler: function accountReset(request, reply) {
        ...
        var account, sessionToken, keyFetchToken, verifiedStatus, verifyHash, wrapKb

        return getVerificationStatus(sessionTokenId)
          .then(verifyAndResetAccount)
          .then(createSessionToken)
          .then(createKeyFetchToken)
          .then(createResponse)
          .done(reply, reply)

        function getVerificationStatus(sessionTokenId) {
          return Tokens.SessionToken.fromHex(sessionTokenId)
            ...
            .then(
              function (tokenId) {
                return db.sessionTokenWithVerificationStatus(Buffer(tokenId, 'hex'))
              }
            )
            .then(
              function (tokenData) {
                verifiedStatus = tokenData.tokenVerificationId ? false : true
              }
            )
            .catch(
              function () {
        ...
        function verifyAndResetAccount () {
          return password.verifyHash()
            .then(
        ...
        function createSessionToken () {
          ...
          var sessionTokenOptions = {
            ...
            emailVerified: account.emailVerified,
            verifierSetAt: account.verifierSetAt,
            tokenVerificationId: verifiedStatus ? undefined : crypto.randomBytes(16)
          }
