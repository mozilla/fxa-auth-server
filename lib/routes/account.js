  ...
  const routes = [
    ...
    },
    {
      ...
      handler: function accountReset(request, reply) {
        ...
        let account, sessionToken, keyFetchToken, verifyHash
        ...
        function checkRecoveryKey() {
          if (recoveryKeyId && wrapKb) {
            return db.getRecoveryKey(accountResetToken.uid, recoveryKeyId)
          }

          if (recoveryKeyId && ! wrapKb || wrapKb && ! recoveryKeyId) {
        ...
        function resetAccountData () {
          ...
          return random.hex(32)
            ...
            .then((verifyHashData) => {
              ...
              // If this is an account recovery, reuse the specified wrapKb instead of creating
              // a random one. The wrapKb specified should contain the original kB of the account.
              if (recoveryKeyId) {
            ...
            .then(() => {
              return P.all([
                ...
                password.unwrap(wrapWrapKb)
              ])
            })
            .then((results) => wrapKb = results[3])
        }
