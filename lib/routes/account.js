    ...
    },
    {
      ...
      handler: function (request, reply) {
        ...
        function skipTokenVerification (request, account) {
          // If they have a cookie from previously using a verified session,
          // they can skip verification for their new session.
          // XXX TODO: are the cookie timestamps authenticated, or just contents?
          log.info({ op: 'request.state', state: request.state })
          if (request.state && request.state.verifiedUid && request.state.verifiedUid === account.uid) {
            return true
          }
