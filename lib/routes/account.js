  ...
  var routes = [
    ...
    },
    {
      ...
      config: {
        ...
        validate: {
          payload: {
            ssoId: isA.string().required(),
            provider: isA.string().required()
          }
        }
      },
      handler: function (request, reply) {
        var ssoId = request.payload.ssoId
        ...
        db.ssoRecord(ssoId)
          .then(
            function (record) {
              password = new Password(record.kA, record.authSalt, record.verifierVersion)
              return record
            },
            function (err) {
                    ...
                    return db.createAccount(
                      {
                        uid: uid,
                        createdAt: Date.now(),
                        email: uid.toString('hex') + '@accounts.firefox.com',
                        emailCode: crypto.randomBytes(16),
                        emailVerified: true,
                        kA: kA,
                        wrapWrapKb: crypto.randomBytes(32),
          ...
          .then(
            function (account) {
              log.info({op: 'Account.sso', account: account })
              return db.createSessionToken(
                {
                  uid: account.uid,
                  email: account.email,
