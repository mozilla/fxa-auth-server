  ...
  var routes = [
    {
      ...
      handler: function accountCreate(request, reply) {
        ...
        request.plugins.customs.check(request, email, 'accountCreate')
    ...
    },
    {
      ...
      handler: function (request, reply) {
        ...
        checkIsBlockForced()
          .then(() => request.plugins.customs.check(request, email, 'accountLogin'))
        ...
        function readEmailRecord () {
          return db.emailRecord(email)
            .then(
              ...
              function (err) {
                if (err.errno === error.ERRNO.ACCOUNT_UNKNOWN) {
                  request.plugins.customs.flag(request.app.clientAddress, {
                    email: email,
                    errno: err.errno
                  })
        ...
        function checkEmailAndPassword () {
          ...
          return request.plugins.password.check(emailRecord, authPW, request.app.clientAddress)
            .then(
              function (match) {
                if (! match) {
    ...
    },
    {
      ...
      handler: function (request, reply) {
        ...
        return request.plugins.customs.checkAuthenticated(endpointAction, ip, stringUid)
          .then(function () {
            if (body.to === 'all') {
    ...
    },
    {
      ...
      handler: function (request, reply) {
        ...
        return request.plugins.customs.check(
          request,
          sessionToken.email,
        ...
        db.account(uid)
          .then(
            (account) => {
              ...
              return request.plugins.customs.check(request, account.email, 'recoveryEmailVerifyCode')
                .then(
                  () => account
                )
            }
    ...
    },
    {
      ...
      handler: function (request, reply) {
        ...
        return request.plugins.customs.check(request, email, 'sendUnblockCode')
          .then(lookupAccount)
      ...
      handler: function accountReset(request, reply) {
        ...
        function resetAccountData () {
          ...
          return random(64)
            ...
            .then(
              function () {
                return request.plugins.customs.reset(account.email)
              }
            )
            .then(
              function () {
    ...
    },
    {
      ...
      handler: function accountDestroy(request, reply) {
        ...
        request.plugins.customs.check(
          ...
          form.email,
          'accountDestroy')
          .then(() => db.emailRecord(form.email))
          .then(
