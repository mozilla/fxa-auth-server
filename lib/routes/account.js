  ...
  var routes = [
    ...
    },
    {
      ...
      handler: function (request, reply) {
        ...
        var metricsContextValid = metricsContext.validate(request)
        if (config.metrics.force_flow_validation && metricsContextValid === false) {
          // if flow validation is on, metrics context is not valid and reason is signin then drop request.
          if (!request.payload.reason || request.payload.reason === 'signin') {
            log.info({
              op: 'Account.login.badMetricsRequest',
              agent: request.headers['user-agent'],
              clientAddress: request.app.clientAddress
            })

            // flag the blocked ip in customs
