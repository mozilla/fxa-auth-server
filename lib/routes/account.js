  ...
  var routes = [
    ...
    },
    {
      ...
      config: {
        ...
        response: {
          schema: isA.array().items(isA.object({
            ...
            lastAccessTimeFormatted: isA.string().allow(null),
            // We previously allowed devices to register with arbitrary unicode names,
            // so we can't assert DISPLAY_SAFE_UNICODE in the response schema.
      ...
      handler: function (request, reply) {
        ...
        var language = request.query.language || 'en'
        var sessionToken = request.auth.credentials
        var uid = sessionToken.uid
        db.devices(uid).then(
          function (deviceArray) {
            reply(deviceArray.map(function (device) {
              ...
              var lastAccessTime = moment(device.lastAccessTime)
              lastAccessTime.locale(language)
              device.lastAccessTimeFormatted = lastAccessTime.fromNow()
