  ...
  var routes = [
          ...
          if (! account.emailVerified) {
            ...
            .then(
              function onSuccess() {
                ...
                return log.flowEvent('account.verifyCodeSent', request)
              },
          ...
          if (doSigninConfirmation) {
            ...
            return getGeoData(ip)
              .then(
                ...
                function () {
                  return log.flowEvent('account.confirmationCodeSent', request)
                }
              )
          }
        }
    ...
    },
    {
      ...
      handler: function (request, reply) {
        ...
        var sessionToken = request.auth.credentials
        var reason = (request.query && request.query.reason) || 'unknown'
        ...
        if (sessionToken.emailVerified) {
          // If they're checking status, and the account *is* verified,
          // log an event with the reason.  This shouldn't happen very
          // many times per user.
          log.activityEvent('account.verified.checkedBecause.' + reason, request, {
            uid: sessionToken.uid.toString('hex')
          })
        }
                ...
                .then(function () {
                  ...
                  return db.verifyEmail(account)
                    ...
                    .then(function () {
                      ...
                      push.notifyUpdate(uid, 'accountVerify').then(function (numSent) {
                        if (numSent > 0) {
                          return log.activityEvent('account.verified.notifedWithPush', fakeRequestObject, {
                            uid: uidHex
                          })
                        }
                      })
