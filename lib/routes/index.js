...
module.exports = function (
  ...
  const clients = require('../clients')(log, db, config)
  ...
  const clientsInstances = require('./clients-instances')(log, db, config, customs, clients, pushbox, push)
  const devicesImpl = require('../devices')(log, db, push)
  const devicesSessions = require('./devices-and-sessions')(log, db, config, customs, push, pushbox, devicesImpl, clients)
  const emails = require('./emails')(log, db, mailer, config, customs, push, clients)
  ...
  allRoutes.forEach(r => {
    ...
    const isOnlyOauthAuthenticated = auth && auth.strategy && (auth.strategy === 'oauthToken' || auth.strategy === 'refreshToken')
    if (auth && ! auth.hasOwnProperty('payload') && ! isOnlyOauthAuthenticated) {
