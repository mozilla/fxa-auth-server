  ...
  var routes = [
    ...
    },
    {
      ...
      handler: async function (request, h) {
        ...
        await customs.check(request, email, 'passwordForgotSendCode')

        const accountRecord = await db.accountRecord.bind(db, email)
        ...
        const passwordForgotToken = await  db.createPasswordForgotToken(accountRecord)
        const emails = db.accountEmails(passwordForgotToken.uid)
        ...
        return P.all([
          ...
          .then(async () => {
            if (butil.buffersAreEqual(passwordForgotToken.passCode, code) &&
              ...
              const accountResetToken = await db.forgotPasswordVerified(passwordForgotToken)
