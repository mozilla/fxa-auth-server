    ...
    },
    {
      ...
      handler: async function (request) {
        ...
        var account, verifyHash, sessionToken, keyFetchToken, verifiedStatus,
          targetsToNotify, originatingDeviceId, hasTotp = false

        return checkTotpToken()
          .then(getSessionVerificationStatus)
          .then(fetchTargetsToNotify)
          .then(changePassword)
          .then(notifyAccount)
        ...
        function fetchTargetsToNotify() {
          // We fetch the targets to notify before changePassword() because
          // db.resetAccount() deletes all the targets saved in the account.
          return clients.accountPushClientsAndDevices(passwordChangeToken.uid).then(targets => {
            ...
            if (originatingDeviceId) {
              targetsToNotify = targetsToNotify.filter(d => (d.id !== originatingDeviceId))
            }
          })
        }
        ...
        function notifyAccount() {
          if (targetsToNotify) {
            ...
            push.notifyPasswordChanged(passwordChangeToken.uid, targetsToNotify)
          }
