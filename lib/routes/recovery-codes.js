...
module.exports = (log, db, config, customs, mailer) => {
  ...
  const RECOVERY_CODE_COUNT = codeConfig && codeConfig.count || 10
  ...
  return [
    {
      ...
      config: {
        ...
        response: {
          schema: {
            recoveryCodes: isA.array().items(isA.string().regex(BASE_36)).length(RECOVERY_CODE_COUNT).optional()
          }
        }
      },
    ...
    },
    {
      ...
      config: {
        ...
        validate: {
          payload: {
            code: isA.string().length(RECOVERY_CODE_LENGTH).regex(BASE_36).required(),
