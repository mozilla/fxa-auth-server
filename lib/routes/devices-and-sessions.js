        ...
        validate: {
          payload: isA.alternatives().try(
            isA.object({
              ...
              encryptedPayload: isA.string().regex(HEX_STRING).when('_endpointAction', { is: 'sendtab', then: isA.required(), otherwise: isA.forbidden() }), // TODO: THIS SHOULD BE BASE64! See https://docs.google.com/document/d/1YT6gh125Tu03eM42Vb_LKjvgxc4qrGGZsty1_ajf2YM/edit?disco=AAAABrAK_y0
              pushboxToken: isA.string().regex(HEX_STRING).when('encryptedPayload', { is: isA.exist(), then: isA.required(), otherwise: isA.forbidden() }),
      ...
      handler (request, reply) {
        ...
        return customs.checkAuthenticated(endpointAction, ip, uid)
          ...
          .then(devices => {
            ...
            if (usePushbox) {
              var oAuthToken = body.pushboxToken
            ...
            return push.notifyUpdate(uid, devices, endpointAction, pushOptions)
