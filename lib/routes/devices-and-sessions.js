...
const PUSHBOX_RETRIEVE_SCHEMA = require('../pushbox').RETRIEVE_SCHEMA
const PUSH_PAYLOADS_SCHEMA_PATH = path.resolve(__dirname, '../../docs/pushpayloads.schema.json')

const MESSAGE_TOPIC_TO_PUSH_REASON = {
  sendtab: 'sendTab'
}
...
module.exports = (log, db, config, customs, push, pushbox, devices) => {
  ...
  return [
    ...
    {
      method: 'GET',
      path: '/account/device/messages',
      config: {
        validate: {
          query: {
            index: isA.string().alphanum().optional(),
            limit: isA.number().optional().min(0).max(100),
          }
        },
        auth: {
          strategy: 'sessionToken'
        ...
        response: {
          schema: PUSHBOX_RETRIEVE_SCHEMA
        }
      },
      handler (request, reply) {
        log.begin('Account.deviceMessages', request)
        ...
        const {index} = query
        const limit = query.limit !== undefined ? Math.min(100, query.limit) : 100

        return customs.checkAuthenticated('deviceMessages', ip, uid)
          .then(() => pushbox.retrieve(uid, deviceId, limit, index))
          .then(reply, reply)
      }
    },
    {
      method: 'POST',
      path: '/account/devices/messages',
      config: {
        auth: {
          strategy: 'sessionToken'
        },
        validate: {
          payload: {
            to: isA.string().length(32).regex(HEX_STRING).required(),
            topic: isA.string().valid('sendtab').required(),
      ...
      handler (request, reply) {
        log.begin('Account.devicesMessages', request)

        const body = request.payload
        const sessionToken = request.auth.credentials
        const uid = sessionToken.uid
        const ip = request.app.clientAddress
        const {to, data, topic} = body
        const pushReason = MESSAGE_TOPIC_TO_PUSH_REASON[topic]

        if (! pushReason) {
          return reply(error.invalidRequestParameter('invalid topic'))
        }

        return customs.checkAuthenticated('sendDeviceMessage', ip, uid)
          .then(() => request.app.devices)
          .then(devices => {
            const device = devices.find(d => d.id === to)
            if (! device) {
              log.error({
                op: 'Account.devicesMessages',
                deviceId: to,
                uid: uid,
            ...
            return pushbox.store(uid, device.id, topic, data)
              .then(url => push.notifyMessageReceived(uid, device, url, topic, pushReason))
          })
          .then(
            () => reply({}),
            reply
