...
module.exports = (log, db, config, customs, push, pushbox, devices, oauthdb) => {
  ...
  function buildDeviceResponse(sessionToken, device = null) {
    ...
    return {
      ...
      availableCommands: (device && device.availableCommands) || sessionToken.deviceAvailableCommands || {},
    }
  }
        ...
        return request.app.devices
          ...
          .then(() => {
            if (credentials.refreshTokenId) {
              ...
              return oauthdb.revokeRefreshToken(credentials.refreshTokenId).catch((err) => {
                log.error('deviceDestroy.revokeRefreshToken.error', {err: err.message})
              })
            }
          })
          .then(() => {
