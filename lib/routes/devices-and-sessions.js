...
module.exports = (log, db, config, customs, push, pushbox, devices, clients) => {
  ...
  return [
    ...
    },
    {
      ...
      async handler(request) {
        ...
        await customs.checkAuthenticated('invokeDeviceCommand', ip, uid)
        const target = await clients.findDeviceOrClientInstance(uid, targetId)
        ...
        const {index} = await pushbox.store(uid, target.id, data, ttl)
    ...
    },
    {
      ...
      async handler(request) {
        ...
        const {id: sessionTokenId} = request.auth.credentials
        return [
          ...(await request.app.devices).map(device => {
            return {
              id: device.id,
              isCurrentDevice: device.sessionToken === sessionTokenId,
          ...
          ...(await clients.getClientsInstances(uid)).map(instance => {
            return {
              id: instance.id,
              isCurrentDevice: false,
              lastAccessTime: null,
              name: instance.name || '',
              type: 'oauth', // TODO: we should try to infer this.
              pushCallback: instance.pushEndpoint,
              pushPublicKey: instance.pushPublicKey,
              pushAuthKey: instance.pushAuthKey,
	...
	return [
          ...(await db.sessions(uid)).map(session => {
          ...
          // TODO: These are not technically "sessions" and we shouldn't
          // return them here; it's a temporary measure to get them to
          // show up in the "devices and apps" list in content-server,
          // pending a content-server fix.
          ...(await clients.getClientsInstances(uid)).map(instance => {
            return {
              deviceId: instance.id,
              deviceName: instance.name || '',
              deviceType: 'oauth',
              deviceAvailableCommands: instance.availableCommands,
              deviceCallbackURL: instance.pushEndpoint,
              deviceCallbackPublicKey: instance.pushPublicKey,
              deviceCallbackAuthKey: instance.pushAuthKey,
