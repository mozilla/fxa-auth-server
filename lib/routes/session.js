  ...
  const routes = [
    {
      ...
      handler: function (request, reply) {
        ...
        return checkCustomsRules()
          .then(readAccountRecord)
          .then(checkAccountPassword)
        ...
        function checkCustomsRules() {
          return signinUtils.checkCustomsRules(request, sessionToken.email)
        }

        function readAccountRecord() {
          return db.accountRecord(sessionToken.email).then((result) => {
        ...
        function checkAccountPassword() {
          return signinUtils.checkPassword(accountRecord, password, request.app.clientAddress)
            .then(match => {
              if (! match) {
                throw error.incorrectPassword(accountRecord.email, sessionToken.email)
              }
            })
        }

        function updateSessionToken() {
          // NB: update mustVerify if needed for the request
          // NB: update lastAuth timestamp somehow
          throw new Error('not implemented yet')
        }

        function createKeyFetchToken() {
          if (requestHelper.wantsKeys(request)) {
            return signinUtils.createKeyFetchToken(accountRecord, password, sessionToken)
              .then(result => {
        ...
        function sendSigninNotifications() {
          return signinUtils.sendSigninNotifications(request, accountRecord, sessionToken)
        }
        ...
        function createResponse () {
          ...
          Object.assign(response, signinUtils.getSessionVerificationStatus(sessionToken))
