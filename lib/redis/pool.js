...
module.exports = (config, log) => {
      ...
      return new P((resolve, reject) => {
        ...
        const client = redis.createClient(Object.assign({}, redisConfig, {
          retry_strategy ({ attempt }) {
            if (attempt <= config.redis.retryCount) {
              return config.redis.initialBackoff * Math.pow(2, attempt - 1)
            }
  ...
  const acquireTimeoutMillis = new Array(config.redis.retryCount)
    .fill(0)
    .reduce((total, _, index) => {
      return total + config.redis.initialBackoff * Math.pow(2, index)
    }, 0)
  ...
  const pool = genericPool.createPool(redisFactory, {
    ...
    min: config.redis.minConnections,
    Promise: P,
    testOnBorrow: true
  })
