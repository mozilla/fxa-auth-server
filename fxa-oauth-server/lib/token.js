...
exports.verify = async function verify(tokenId, type = 'access_token') {
  const tokenHash = encrypt.hash(tokenId);
  let token;
  if (type === 'refresh_token') {
    token = await db.getRefreshToken(tokenHash);
  } else {
    token = await db.getAccessToken(tokenHash);
  }
  if (! token) {
  ...
  if (token.instanceId) {
    tokenInfo.instance_id = token.instanceId.toString('hex');
  }
